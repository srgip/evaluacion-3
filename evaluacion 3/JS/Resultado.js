...
function completarFila(element,index,arr){
    ...
    function obtenerUsuario(){
     ...
     function obtenerParametroResultadoEliminar(){
        ...
        obtenerDatosResultadoEliminar(p_id_gestion);   
    }
    function obtenerDatosResultadoEliminar(p_id_Resultado){
        const requestOptions = {
          ...
          fetch("http://144.126.210.74:8080/api/gestion/"+p_id_Resultado, requestOptions)
            .then((response) => response.json())
    ...
    function completarEtiquetaEliminar(element,index,arr){
        ...
        var fecha_Resultado = formatearFecha(element.fecha_registro);
        var etiquetaEliminar = document.getElementById('lbl_eliminar');
        etiquetaEliminar.innerHTML = "<p>¿Seguro que desea eliminar esta Resultado? ID: " + id_gestion + " de fecha "+ fecha_gestion+"</p>";
